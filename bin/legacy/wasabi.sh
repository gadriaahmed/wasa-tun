...
start() {
  ./bin/legacy/container.sh -v ${verify} -m ${migration} start${1:+:$1}
}
  ...
  for resource in $1; do
    case "${1}" in
      ...
      mysql|cassandra) ./bin/wasabi.sh status 2>/dev/null | grep wasabi-${1} 1>/dev/null || ./bin/wasabi.sh start
        ./bin/legacy/container.sh console:${1};;
...
stop() {
  ./bin/legacy/container.sh stop${1:+:$1}
}

status() {
  ./bin/legacy/container.sh status
}
...
package() {
  ...
  echo "WASABI.SH start: ./bin/legacy/fpm.sh -n ${name} -v ${version} -p ${profile} "
  ./bin/legacy/fpm.sh -n ${name} -v ${version} -p ${profile}
  echo "WASABI.SH end: ./bin/legacy/fpm.sh "
...
#    ; grunt test); \
    ...
    (cd target; ../bin/legacy/fpm.sh -n ${name} -v ${version} -p ${profile}))
...
remove() {
  ./bin/legacy/container.sh remove${1:+:$1}
}
